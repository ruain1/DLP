...
const fromActivity = (r) => ({
  ...
  subArea: r.sub_area || "", tier3: r.tier3 || "", asset: r.asset || "", discipline: (r.discipline ? String(r.discipline).split(/\s*;\s*/).filter(Boolean) : []), witnessInvite: !!r.witness_invite, witnessType: r.witness_type || "", witnessAt: r.witness_at || "", witnessDurationMin: (r.witness_duration_min == null ? 60 : Number(r.witness_duration_min)), witnessDays: (r.witness_days == null ? 1 : Math.max(1, Number(r.witness_days))), witnessSentAt: r.witness_sent_at || "", notes: r.notes || "", slipReason: r.slip_reason || "", accUrl: r.acc_url || "", fokRef: r.fok_ref || "",
...
export async function loadAll(session, projectId, projectName) {
  ...
  return {
    ...
    levels: levelsObj,
    settings: { weeks: settings.data?.weeks ?? 4, makeReadyDays: settings.data?.make_ready_days ?? 7, ppcTarget: settings.data?.ppc_target ?? 80 },
...
export async function syncCollections(prev, next, session, projectId) {
  ...
  if (next.settings !== prev.settings) {
    ops.push(supabase.from("settings").update({ weeks: next.settings.weeks, make_ready_days: next.settings.makeReadyDays, ppc_target: next.settings.ppcTarget ?? 80 }).eq("project_id", projectId));
  }
