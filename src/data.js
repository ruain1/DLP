...
export async function loadAll(session, projectId, projectName) {
  ...
  return {
    ...
    levels: levelsObj,
    settings: { weeks: settings.data?.weeks ?? 4, makeReadyDays: settings.data?.make_ready_days ?? 7, ppcTarget: settings.data?.ppc_target ?? 80, benchmarksVisible: settings.data?.benchmarks_visible ?? false },
...
export async function syncCollections(prev, next, session, projectId) {
  ...
  if (next.settings !== prev.settings) {
    ops.push(supabase.from("settings").update({ weeks: next.settings.weeks, make_ready_days: next.settings.makeReadyDays, ppc_target: next.settings.ppcTarget ?? 80, benchmarks_visible: !!next.settings.benchmarksVisible }).eq("project_id", projectId));
  }
