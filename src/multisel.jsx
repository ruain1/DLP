...
export function MultiSel({ label, options, value, onChange, minWidth }) {
  ...
  const C = {
    ...
    btn: { display: "inline-flex", alignItems: "center", gap: 8, background: "var(--card, #141d2c)", border: "1px solid " + (active ? "var(--accent, #5B9BF3)" : "var(--line, #28354a)"), borderRadius: 9, padding: "7px 12px", fontSize: 12.5, fontWeight: 650, color: "var(--ink, #e6edf6)", cursor: "pointer", minWidth: minWidth || 0, whiteSpace: "nowrap" },
    caret: { color: "var(--muted, #8b98ab)", fontSize: 10, lineHeight: 1 },
    pop: { position: "absolute", top: "calc(100% + 7px)", left: 0, minWidth: 200, maxHeight: 320, overflow: "auto", background: "var(--card, #141d2c)", border: "1px solid var(--line, #28354a)", borderRadius: 11, padding: "6px 0", boxShadow: "0 14px 34px rgba(0,0,0,.45)", zIndex: 70 },
    row: { display: "flex", gap: 9, alignItems: "center", padding: "7px 12px", cursor: "pointer", fontSize: 12.5, color: "var(--ink, #e6edf6)", userSelect: "none" },
    box: (on) => ({ width: 15, height: 15, borderRadius: 4, flex: "0 0 auto", display: "inline-flex", alignItems: "center", justifyContent: "center", border: "1.5px solid " + (on ? "var(--accent, #5B9BF3)" : "var(--line2, #3b4a5e)"), background: on ? "var(--accent, #5B9BF3)" : "transparent", color: "#fff", fontSize: 10, fontWeight: 900, lineHeight: 1 }),
    div: { height: 1, background: "var(--line, #28354a)", margin: "4px 0" },
  };
