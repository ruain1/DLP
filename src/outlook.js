...
// The app's own address, taken from the running page so emails always point at
// whichever domain the sender is actually using, before and after the domain move.
const APP_ORIGIN = (typeof window !== "undefined" && window.location && window.location.origin) || "https://dlp-pi.vercel.app";
...
export function buildInviteBodyHtml(p) {
  ...
  return `<table role="presentation" cellpadding="0" cellspacing="0" border="0" width="560" style="border-collapse:collapse;background-color:#ffffff;border:1px solid #d8dee9;${FSTACK}">`
    ...
    + `<td style="padding:12px 18px;font-size:15px;font-weight:bold;color:#ffffff;${FSTACK}">${eH(p.projectName || "FIN04")} Witness Invitation</td>`
  ...
  const card = `<table role="presentation" cellpadding="0" cellspacing="0" border="0" width="780" style="border-collapse:collapse;background-color:#ffffff;border:1px solid #d8dee9;${FSTACK}">`
    + (() => {
        ...
        const markCell = useLogo
          ? `<td style="vertical-align:middle;padding-right:11px;"><img src="${eH(logo)}" alt="atnorth" height="34" style="height:34px;width:auto;display:block;"></td>`
          : `<td style="vertical-align:middle;padding-right:12px;color:#7BB2E8;font-size:18px;font-weight:bold;letter-spacing:-.3px;${FSTACK}">atnorth</td>`;
        return `<tr><td style="padding:0;background-color:#001C26;"><table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%"><tr><td style="padding:20px 20px 18px 20px;"><table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%"><tr>`
          + `<td style="vertical-align:middle;"><table cellpadding="0" cellspacing="0"><tr>${markCell}`
          + `<td style="vertical-align:middle;"><div style="color:#5C7690;font-size:9.5px;letter-spacing:.18em;font-weight:bold;${FSTACK}">WEEKLY DLP REPORT</div>`
          + `<div style="color:#ffffff;font-size:19px;font-weight:bold;letter-spacing:-.3px;padding-top:2px;${FSTACK}">${eH(p.projectName || "FIN04")}</div></td></tr></table></td>`
          + `<td align="right" style="vertical-align:middle;"><div style="display:inline-block;font-family:'Courier New',monospace;font-size:10px;letter-spacing:.1em;color:#7BB2E8;border:1px solid #33586e;border-radius:4px;padding:3px 8px;">${eH(p.projectName || "FIN04")}</div>`
          + `<div style="color:#9DB0C2;font-size:11px;padding-top:7px;${FSTACK}">${eH(p.periodLabel)}<br>generated by ${eH(p.by || "")}</div></td>`
          + `</tr></table></td></tr>`
