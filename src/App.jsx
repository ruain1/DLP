...
const THEMES = {
  light: { ink: "#16202E", paper: "#F7F8FA", card: "#FFFFFF", line: "#DCE1E8", muted: "#6B7785", accent: "#1E5FCC", head: "#2563EB", weekend: "#EFF1F5", todcell: "#F0F5FE", todhead: "#E8EFFB", todedge: "#A3BEEC", hover: "#EAF0F9", chipbg: "#E8EFFB" },
...
.lk-rep-filters{display:flex;flex-wrap:wrap;gap:12px;align-items:flex-end;margin:0 0 14px;position:sticky;top:0;z-index:20;background:var(--card);padding:10px 0;border-bottom:1px solid var(--line)}
...
.lk-rep-sec h3{font-size:12.5px;font-weight:700;margin:0 0 12px;text-transform:uppercase;letter-spacing:.06em;color:var(--head)}
...
.lk-bar-row.pareto{grid-template-columns:140px 1fr 30px 42px 46px}
.lk-bar-row.pareto .ps{text-align:right;color:var(--muted)}
.lk-bar-row.pareto .pc{text-align:right;font-weight:600;color:#14B8A6}
.lk-bar-head{display:grid;grid-template-columns:140px 1fr 30px 42px 46px;gap:10px;font-size:9.5px;text-transform:uppercase;letter-spacing:.06em;color:var(--muted);margin:0 0 5px}
.lk-bar-head span:nth-child(n+3){text-align:right}
.lk-mix-row{display:grid;grid-template-columns:110px 1fr 30px 46px 40px;gap:10px;align-items:center;margin-bottom:7px;font-size:12px}
.lk-mix-row .nm{white-space:normal;word-wrap:break-word;line-height:1.2}
.lk-mix-row .cn{text-align:right;font-weight:600}
.lk-mix-row .cp{text-align:right;font-weight:700}
.lk-mix-row .co{text-align:right;color:var(--muted)}
.lk-mix-row .co.warn{color:#D97706;font-weight:700}
.lk-mix-row.clickable{cursor:pointer;border-radius:6px;transition:background .12s}
.lk-mix-row.clickable:hover{background:var(--hover)}
.lk-mix-head{display:grid;grid-template-columns:110px 1fr 30px 46px 40px;gap:10px;font-size:9.5px;text-transform:uppercase;letter-spacing:.06em;color:var(--muted);margin:0 0 5px}
.lk-mix-head span:nth-child(n+3){text-align:right}
          ...
          {dispOpen && <div className="lk-dispfly">
            ...
            {[["default", "Default", ["#18b69b", "#e0a106", "#e2564e"]], ["hc", "High contrast", ["#0E9E6E", "#B45309", "#D62828"]], ["cb", "Colourblind safe", ["#0072B2", "#E0A106", "#D55E00"]]].map(([k, lbl, cols]) => (
              <button key={k} className="lk-dispopt" onClick={() => { update((pp) => ({ ...pp, palette: k })); setDispOpen(false); }}>
                <span className={"lk-displbl" + (palette === k ? " on" : "")}>{lbl}</span><span className="lk-dispsw">{cols.map((c, i) => <i key={i} style={{ background: c }} />)}</span>
              </button>
            ))}
          </div>}
        </div>
...
<select className="lk-wsel" value={S.viewWeeks || (S.settings && S.settings.weeks) || 4} title="Board window: how far ahead your board draws. This is your own view preference and does not change the project or anyone else's board." onChange={(e) => setS((p) => ({ ...p, viewWeeks: Number(e.target.value) }))}>
      ...
      <div className="lk-board">
        ...
        {S.view === "swimlane" && lanesList.map((lane) => {
          ...
          // row is free from the retest's start, so an attempt chain (card, failure tail, connector,
          // retest) reads as a single horizontal line. The parent is always packed first (earlier start,
          // and retests win same-day ties in the sort above), so its row is known when the retest lands.
...
function AdminPanel({ S, cu, update, exportActivities, can, isOwner, projClient }) {
  ...
  const emailUserInvite = async ({ email, roleLabel, companyName, link, mode }) => {
    if (!/@/.test(email || "")) { setUserMsg("No email address on file for " + (email || "this account") + " (the status lookup may still be loading); use Copy invite details, or reopen this card in a moment."); return null; }
    const ol = await import("./outlook");
    const acct = await ol.outlookAccount();
...
function Trend({ points, h = 168, onPoint, target }) {
  ...
  // The open (in-progress) week draws hollow with a dashed lead-in; closed weeks carry the solid line and the area fill.
  const closed = valid.filter((p) => !p.open);
  const openPt = valid.find((p) => p.open) || null;
  const line = closed.map((p, k) => `${k === 0 ? "M" : "L"}${xs(p.i)},${ys(p.value)}`).join(" ");
  const area = closed.length ? `${line} L${xs(closed[closed.length - 1].i)},${padT + ih} L${xs(closed[0].i)},${padT + ih} Z` : "";
  const dash = openPt && closed.length ? `M${xs(closed[closed.length - 1].i)},${ys(closed[closed.length - 1].value)} L${xs(openPt.i)},${ys(openPt.value)}` : "";
  // 4-week rolling average, computed upstream over summed kept/due, not averaged percentages
  const avgPts = points.map((p, i) => ({ v: p.avg, i })).filter((p) => p.v != null);
  const avgLine = avgPts.length > 1 ? avgPts.map((p, k) => `${k === 0 ? "M" : "L"}${xs(p.i)},${ys(p.v)}`).join(" ") : "";
  const tip = (p) => p.label + ": " + p.value + "%" + (p.due != null ? " (" + p.kept + " of " + p.due + " kept" + (p.open ? ", week in progress" : "") + ")" : "");
  return <>
  <svg viewBox={`0 0 ${w} ${h}`} width="100%" style={{ height: "auto", display: "block" }}>
    ...
    {avgLine && <path d={avgLine} fill="none" stroke="#F59E0B" strokeWidth="1.8" strokeDasharray="2 4" strokeLinejoin="round" />}
    {closed.map((p) => <circle key={p.i} cx={xs(p.i)} cy={ys(p.value)} r="3.5" fill="var(--accent)" />)}
    {openPt && <circle cx={xs(openPt.i)} cy={ys(openPt.value)} r="4" fill="var(--card)" stroke="var(--accent)" strokeWidth="2" />}
...
// RepBar: pass share and cum (both 0-100) to render Pareto columns, with the bar scaled to share of
// the total rather than the biggest bar. Without them it renders the plain ranked form used by
// By Company and By Cx Stage. Labels wrap instead of clipping, completing REV102 on screen.
const RepBar = ({ label, n, max, color, onClick, share, cum }) => {
  const pareto = share != null;
  const w = pareto ? share : Math.round((n / max) * 100);
  return <div className={"lk-bar-row" + (pareto ? " pareto" : "") + (onClick ? " clickable" : "")} onClick={onClick}>
    <span style={{ whiteSpace: "normal", wordWrap: "break-word", lineHeight: 1.25 }}>{label}</span>
    <div className="lk-bar-track" style={pareto ? { position: "relative" } : undefined}>
      <div className="lk-bar-fill" style={{ width: `${w}%`, background: color || "var(--accent)" }} />
      {cum != null && <div title={"Cumulative " + cum + "%"} style={{ position: "absolute", top: 0, bottom: 0, left: cum >= 100 ? "calc(100% - 2px)" : `calc(${cum}% - 1px)`, width: 2, background: "#14B8A6" }} />}
    </div>
    <span className="n">{n}</span>
    {pareto && <span className="ps">{share}%</span>}
    {pareto && <span className="pc">{cum}%</span>}
  </div>;
};
const ParetoHead = ({ first = "Reason" }) => <div className="lk-bar-head"><span>{first}</span><span /><span>N</span><span>Share</span><span>{"\u03A3"}</span></div>;
...
const MixRow = ({ swatch, label, n, max, done, inprog, pct, tip, open, onClick }) => {
  ...
  return <div className="lk-mix-row clickable" onClick={onClick} title={tip}>
    ...
    <span className="cp" style={{ color: ppcColor(pct) }}>{pct == null ? "n/a" : pct + "%"}</span>
    <span className={"co" + (open > 0 ? " warn" : "")}>{open}</span>
  </div>;
};
...
function buildWeeklyReportHTML({ r, summary, includeSchedule, by, mode, theme, sections, cxSectionsHtml }){
  ...
  // reasons: Pareto form, bar widths are share of all misses, with running cumulative
  const totR = r.reasons.reduce((s,x)=>s+x.n,0);
  const bhead = `<div class="barhead"><span>Reason</span><span></span><span>N</span><span>Share</span><span>&#931;</span></div>`;
  let _cr = 0;
  const reasonsHtml = r.reasons.length ? bhead + r.reasons.map((x)=>{ _cr += x.n; const sh = totR?Math.round(x.n/totR*100):0, cu = totR?Math.round(_cr/totR*100):0; return `<div class="barrow p5"><span class="nm">${esc(x.name)}</span><div class="track" style="position:relative"><div class="fill" style="width:${sh}%;background:var(--red)"></div><div style="position:absolute;top:0;bottom:0;left:${cu>=100?"calc(100% - 2px)":`calc(${cu}% - 1px)`};width:2px;background:var(--green)"></div></div><span class="ct num">${x.n}</span><span class="psh num">${sh}%</span><span class="pcu num">${cu}%</span></div>`; }).join("")
...
body.dark .b-will{background:#1D4ED8;color:#DBE7FB}
body.dark .b-wit{background:#7C3AED;color:#EDE9FE}
...
.badge{font-size:10.5px;font-weight:700;letter-spacing:.04em;padding:4px 9px;border-radius:999px;white-space:nowrap}.b-will{background:#1D4ED8;color:#DBE7FB}.b-wit{background:#7C3AED;color:#EDE9FE}
...
.barrow{display:grid;grid-template-columns:140px 1fr 26px;gap:10px;align-items:center;font-size:12.5px}.barrow .nm{color:var(--ink-2);white-space:normal;overflow:visible;word-wrap:break-word;line-height:1.25}
.barrow.p5{grid-template-columns:140px 1fr 26px 40px 44px}.barrow .psh{text-align:right;color:var(--ink-2)}.barrow .pcu{text-align:right;font-weight:600;color:var(--green)}
.barhead{display:grid;grid-template-columns:140px 1fr 26px 40px 44px;gap:10px;font-size:9px;text-transform:uppercase;letter-spacing:.06em;color:var(--ink-2);margin-bottom:4px}.barhead span:nth-child(n+3){text-align:right}
    ...
    {open && <div className="lk-modal-bg" onClick={() => setOpen(false)}>
      <div className="lk-modal" style={{ ...cssVars(S.theme), maxWidth: 680 }} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="rep-foot" style={{ flexWrap: "wrap", gap: 8 }}><button className="lk-btn" onClick={() => setOpen(false)}>Cancel</button>
          ...
          <button className="lk-btn" disabled={!rData} onClick={emlReport} title="Download a pre-addressed .eml draft; opening it launches classic Outlook in compose mode with the report attached. Classic Outlook only.">Open In Outlook (Draft)</button>
...
function ReportsPage({ S, LV, coName, exportActivities, onOpen, isAdmin, canWeekly, canDist, by, projectId }) {
  ...
  // trend's old future-weeks bug. Same convention as the Reasons panel and the trend since REV105.
  ...
  // Per-stage progression: how far through each Cx stage, and what is blocking it
  const byLevel = Object.keys(LV).map((k) => {
    const la = acts.filter((a) => a.level === k);
    return { k, name: `${k} ${LV[k].name}`, color: LV[k].color, n: la.length, done: la.filter((a) => a.status === "complete").length, inprog: la.filter((a) => a.status === "in_progress").length, open: la.reduce((s, a) => s + openOf(a), 0) };
  }).filter((x) => x.n > 0);
  // Status mix with actionable sub-splits: can planned work start, is live work healthy,
  // did completions honour their dates (no actual finish recorded counts as on time, per made())
  const onTimeDone = (a) => !a.actualFinish || !a.start || parseD(a.actualFinish) <= finishOf(a);
  const stOf = (k) => acts.filter((a) => a.status === k);
  const stPlanned = stOf("planned"), stProg = stOf("in_progress"), stDone = stOf("complete");
  const statusData = [
  ...
  if (withDates.length) {
    ...
    // Never plot weeks that have not happened: a committed activity due in the future cannot be
    // "complete", so future weeks would render as 0% kept, which is an artifact, not a measurement.
    const curWk = mondayOf(new Date(todayMid()));
    if (end.getTime() > curWk.getTime()) end = curWk;
    let guard = 0;
    while (cur.getTime() <= end.getTime() && guard < 60) {
      ...
      const kept = comm.filter(made).length;
      points.push({ label: "W" + isoWeek(wk), value: comm.length ? Math.round(kept / comm.length * 100) : null, items: comm, kept, due: comm.length, open: addDays(wk, 6).getTime() >= todayMid() });
      cur = addDays(cur, 7); guard++;
    }
    // 4-week rolling average over summed kept/due, not averaged percentages, so a 1-of-1 week
    // cannot swing the average the way it swings its own point
    points.forEach((p, i) => { let k = 0, d = 0; for (let j = Math.max(0, i - 3); j <= i; j++) { k += points[j].kept; d += points[j].due; } p.avg = d ? Math.round((k / d) * 100) : null; });
  }
  ...
  // Pareto columns: share of the total and running cumulative, computed on counts then rounded,
  // so the final row's cumulative is exactly 100%
  const withPareto = (rows, total) => { let c = 0; return rows.map((x) => { c += x.n; return { ...x, share: total ? Math.round((x.n / total) * 100) : 0, cum: total ? Math.round((c / total) * 100) : 0 }; }); };
  const reasonRowsP = withPareto(reasonRows, misses.length);
  ...
  const failRowsP = withPareto(failRows, witFailed.length);
  ...
  const exportMetrics = async () => {
    try {
      ...
      const r = wb.addWorksheet("Non-completion"); r.columns = [{ header: "Reason", key: "n", width: 34 }, { header: "Count", key: "c", width: 10 }, { header: "Share %", key: "s", width: 10 }, { header: "Cumulative %", key: "u", width: 14 }]; head(r);
      reasonRowsP.length ? reasonRowsP.forEach((x) => r.addRow({ n: x.name, c: x.n, s: x.share, u: x.cum })) : r.addRow({ n: "No missed commitments in scope", c: 0 });
      const bc = wb.addWorksheet("By company"); bc.columns = [{ header: "Company", key: "n", width: 26 }, { header: "Activities", key: "a", width: 12 }, { header: "Complete", key: "d", width: 10 }, { header: "In progress", key: "i", width: 12 }, { header: "PPC %", key: "p", width: 10 }, { header: "PPC kept", key: "k", width: 10 }, { header: "PPC due", key: "u", width: 10 }, { header: "Open constraints", key: "o", width: 16 }]; head(bc);
      byCompany.forEach((x) => bc.addRow({ n: x.name, a: x.n, d: x.done, i: x.inprog, p: x.ppcDue ? Math.round((x.ppcKept / x.ppcDue) * 100) : "n/a", k: x.ppcKept, u: x.ppcDue, o: x.open }));
      const bl = wb.addWorksheet("By Cx stage"); bl.columns = [{ header: "Cx stage", key: "n", width: 26 }, { header: "Activities", key: "a", width: 12 }, { header: "Complete", key: "d", width: 10 }, { header: "Complete %", key: "p", width: 12 }, { header: "In progress", key: "i", width: 12 }, { header: "Open constraints", key: "o", width: 16 }]; head(bl);
      byLevel.forEach((x) => bl.addRow({ n: x.name, a: x.n, d: x.done, p: x.n ? Math.round((x.done / x.n) * 100) : 0, i: x.inprog, o: x.open }));
  ...
  return (
    <div className="lk-rep">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(280px,1fr))", gap: 16 }}>
        <div className="lk-rep-sec"><h3>Status Mix</h3><div style={{ display: "flex", gap: 20, alignItems: "center", flexWrap: "wrap" }}><Donut data={statusData} onSlice={(d) => openDrill(d.name, acts.filter((a) => a.status === d.k))} /><div style={{ display: "flex", flexDirection: "column", gap: 2, flex: 1, minWidth: 240 }}>{statusData.map((s) => <div key={s.k}>
          <div onClick={() => openDrill(s.name, acts.filter((a) => a.status === s.k))} style={{ display: "grid", gridTemplateColumns: "12px 90px 34px 44px", gap: 8, alignItems: "center", fontSize: 12.5, cursor: "pointer", padding: "2px 6px", borderRadius: 7 }}>
            <span style={{ width: 11, height: 11, borderRadius: 3, background: s.color }} />
            <span>{s.name}</span>
            <span style={{ textAlign: "right", fontWeight: 700 }}>{s.n}</span>
            <span style={{ textAlign: "right", color: "var(--muted)" }}>{acts.length ? Math.round((s.n / acts.length) * 100) : 0}%</span>
          </div>
          {s.n > 0 && <div style={{ display: "flex", gap: 14, margin: "0 0 7px 26px", fontSize: 11, color: "var(--muted)", flexWrap: "wrap" }}>
            {s.subs.map((u) => <span key={u.label} onClick={() => openDrill(s.name + " \u00b7 " + u.label, u.items)} style={{ cursor: "pointer", borderBottom: "1px dotted var(--line)", paddingBottom: 1 }} title={"Click to drill to the " + u.items.length + " activit" + (u.items.length === 1 ? "y" : "ies")}>
              <span style={{ display: "inline-block", width: 7, height: 7, borderRadius: "50%", marginRight: 5, verticalAlign: "middle", background: u.color }} />
              <b style={{ color: "var(--ink)", fontWeight: 600 }}>{u.items.length}</b> {u.label}
            </span>)}
          </div>}
        </div>)}</div></div></div>
